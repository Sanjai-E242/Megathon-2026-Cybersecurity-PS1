...
import { Action, DecisionType } from '../../types';
import { DecisionBadge } from '../common/DecisionBadge';
import { RiskPill } from '../common/RiskPill';
import { Activity, ShieldAlert, Clock, Database, FileText, CheckCircle2 } from 'lucide-react';
    ...
    <div className="cyber-panel rounded-2xl p-6 border border-slate-700 shadow-xl flex flex-col h-[520px]">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold font-mono text-white">LIVE ACTION FEED</h2>
          <span className="text-[10px] font-mono bg-cyan-950/80 border border-cyan-500/40 text-cyan-300 px-2 py-0.5 rounded">
            REALTIME STREAM
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto mt-4 pr-1">
        {actions.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-8 text-slate-500 font-mono text-xs">
            ...
            <p className="text-[11px] text-slate-600 mt-1">Run a scenario to start the runtime interception stream.</p>
          </div>
        ) : (
                ...
                return (
                  <motion.div
                    ...
                    className={`p-3.5 rounded-xl border text-xs font-mono transition-all cursor-pointer ${
                      isBlocked
                        ? 'bg-rose-950/50 border-rose-500/80 shadow-lg shadow-rose-950/40 ring-1 ring-rose-500/40'
                        : isConfirm
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-cyan-300">{action.operation}</span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {action.drift_score !== undefined && (
                          <span className={`text-[11px] px-2 py-0.5 rounded font-mono ${
                            action.drift_score > 0.75 ? 'bg-rose-950 text-rose-300 border border-rose-500/40' :
                            action.drift_score > 0.45 ? 'bg-amber-950 text-amber-300 border border-amber-500/40' :
                    ...
                    {(action.reason || action.principal_id) && (
                      <div className="mt-2 pt-2 border-t border-slate-800/80 flex flex-wrap items-center justify-between text-[11px] text-slate-400 gap-2">
                        <div className="flex items-center gap-2">
                          <span>Principal: <code className="text-indigo-300 font-semibold">{action.principal_id}</code></span>
                          <span className="text-slate-600">•</span>
                          <span>Scope: <code className="text-slate-300">{action.scope_required}</code></span>
                        </div>
                        {action.reason && (
                          <div className={`italic truncate max-w-md ${
                            isBlocked ? 'text-rose-300 font-medium' : isConfirm ? 'text-amber-300 font-medium' : 'text-slate-400'
                          }`}>
                            {action.reason}
                          </div>
                        )}

...
import { GitCommit, ArrowRight, Flame, Eye, Edit3 } from 'lucide-react';
  ...
  if (actions.length === 0) {
    return (
      <div className="cyber-panel rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-xl text-center text-slate-400 dark:text-slate-500 font-mono text-xs py-12 transition-colors duration-200">
        ...
        <p className="text-[11px] text-slate-400 dark:text-slate-600 mt-1">Execute a scenario to visualize behavioral escalation nodes.</p>
      </div>
    );
  }

  return (
    <div className="cyber-panel rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-xl overflow-x-auto transition-colors duration-200">
      <div className="flex items-center justify-between pb-4 mb-6 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          <GitCommit className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
          <h2 className="text-base font-bold font-mono text-slate-900 dark:text-white">SESSION TRAJECTORY ESCALATION GRAPH</h2>
        </div>
          ...
          return (
            <React.Fragment key={action.action_id || idx}>
              <motion.div
                ...
                className={`p-4 rounded-xl border flex flex-col items-center text-center w-48 relative shadow-sm ${
                  isBlocked
              ...
              >
                {/* Node index */}
                <div className="text-[10px] font-mono text-slate-400 dark:text-slate-500 mb-1">NODE 0{idx + 1}</div>

                {/* Icon */}
                <div className={`p-2 rounded-lg mb-2 ${
                  isBlocked ? 'bg-rose-100 dark:bg-rose-900 text-rose-700 dark:text-rose-300' :
                  isDestructive ? 'bg-amber-100 dark:bg-red-900/50 text-amber-700 dark:text-red-300' :
                  isWrite ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300' :
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-full mt-0.5">
                  {action.target}
                </div>

                <div className="mt-3 pt-2 border-t border-slate-200 dark:border-slate-800/80 w-full flex items-center justify-between">

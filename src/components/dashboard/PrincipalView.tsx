...
import { Users, Check, X, ShieldAlert, KeyRound, Bot, User } from 'lucide-react';
    ...
    <div className="cyber-panel rounded-2xl p-6 border border-slate-700 shadow-xl space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {principals.map((principal) => {
          const isAgent = principal.principal_id.startsWith('agent_');
          const scopes = principal.authorized_scopes || [];

          return (
            <div
              key={principal.principal_id}
              className="p-5 rounded-xl bg-slate-950/80 border border-slate-800 hover:border-indigo-500/40 transition-all shadow-md"
            >
              {/* Top info */}
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                    isAgent ? 'bg-cyan-950 text-cyan-400 border border-cyan-500/40' : 'bg-indigo-950 text-indigo-400 border border-indigo-500/40'
                  }`}>
                    {isAgent ? <Bot className="w-5 h-5" /> : <User className="w-5 h-5" />}
                  </div>
                  <div>
                    <h3 className="font-mono font-bold text-white text-sm">{principal.principal_id}</h3>
                    <span className="text-[11px] font-mono text-slate-400">Role: <span className="text-indigo-300">{principal.role}</span></span>
                  </div>
                </div>

                <span className="text-[10px] font-mono bg-slate-900 px-2 py-0.5 rounded border border-slate-700 text-slate-400">

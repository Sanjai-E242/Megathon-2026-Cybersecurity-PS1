...
import { Shield, Terminal, ArrowRight, Play, Layers } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenConsole, onRunAttackDemo }) => {
  return (
    <header className="fixed top-0 left-0 right-0 z-40 bg-[#06080d]/80 backdrop-blur-md border-b border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-base tracking-wider text-white font-mono">SENTINEL</span>
              <span className="text-[10px] font-semibold uppercase tracking-widest text-cyan-400 bg-cyan-950/80 border border-cyan-500/40 px-1.5 py-0.5 rounded">RUNTIME</span>
            </div>
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-mono text-slate-300">
          ...
          <a href="#layers" className="hover:text-cyan-400 transition-colors">Three Layers</a>
          <a href="#comparison" className="hover:text-cyan-400 transition-colors">Attack vs Admin</a>
        </nav>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-medium text-rose-300 bg-rose-950/40 border border-rose-500/40 hover:bg-rose-900/40 transition-all hover:scale-105"
          >
